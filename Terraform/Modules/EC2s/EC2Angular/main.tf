resource "aws_instance" "ec2" {
  ...
  private_ip = "10.0.16.80"
  tags = {
    Name : "ec2-${var.author_name}-angular"
  }
...
#    command = "echo IP : ${var.public_ip}, ID: ${aws_instance.ec2.id}, Zone: ${aws_instance.ec2.availability_zone} > ${var.main_directory}/ip.host"
  ...
  provisioner "local-exec" {
    command = "echo '[webserver]\n${self.public_ip}' > ${var.main_directory}/hostsAngular.ini"
  ...
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=false ansible-playbook -u ubuntu -b -i ${var.main_directory}/hostsAngular.ini --private-key ${var.private_key_path} ${var.main_directory}playbooks/playbookAngular.yml"
  }
}
