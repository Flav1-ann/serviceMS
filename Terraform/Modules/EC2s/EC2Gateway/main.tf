resource "aws_instance" "ec2" {
  ...
  tags = {
    Name : "ec2-${var.author_name}-gateway"
  }
...
#    command = "echo IP : ${var.public_ip}, ID: ${aws_instance.ec2.id}, Zone: ${aws_instance.ec2.availability_zone} > ${var.main_directory}/ip.host"
  ...
  provisioner "local-exec" {
    command = "echo '[webserver]\n${self.public_ip}' > ${var.main_directory}/hostsGateway.ini"
  ...
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=false ansible-playbook -u ubuntu -b -i ${var.main_directory}/hostsGateway.ini --private-key ${var.private_key_path} ${var.main_directory}playbooks/playbookGateway.yml"
  }
}
